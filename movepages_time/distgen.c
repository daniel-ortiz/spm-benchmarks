...
void * call_move_pages(void * arg){
	...
	if(pages_to_move > pages_size){
		printf("Must increase the size of the domain, not enough pages to move \n");
		return;	
	}
	...
	sleep(PHAS1_TIME);
		...
		for(int i=0; i<pages_size; i++){
				double * offset= mr->start[0]+ i*512;
				pages[i]=(void *) offset;
				nodes[i]=1;
			}
			...
			//We will move as much pages as we need
			t1=wtime();
			int ret= 	move_pages(getpid(), pages_to_move, pages+begin, nodes+begin, status+begin,0);
			diff=wtime()-t1;
			printf("move pages %d -res -%d -in- %f \n", pages_to_move, ret,diff);
...
int main(int argc, char* argv[])
{
  ...
  for(arg=1; arg<argc; arg++) {
    if (argv[arg][0] == 'x') {
		pages_to_move = toInt(argv[arg]+1, 0);
		printf("Will benchmark move of the pages %d \n",pages_to_move);
		continue;
	}
    if (argv[arg][0] == '-') {
